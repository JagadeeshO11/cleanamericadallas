...
export default function App(){const [page,setPage]=useState('login');const [authenticated,setAuthenticated]=useState(false);const [selectedService,setSelectedService]=useState(services[0]);const [selectedBooking,setSelectedBooking]=useState(initialBookings[0]);const [bookings,setBookings]=useState(initialBookings);const addBooking=b=>setBookings(items=>[b,...items]);const logout=()=>{setAuthenticated(false);setPage('login');setSelectedBooking(initialBookings[0])};if(!authenticated)return <Login onSuccess={()=>{setAuthenticated(true);setPage('home')}}/>;let content;if(page==='home')content=<Home services={services} logo={LOGO_URL} setPage={setPage} setSelectedService={setSelectedService}/>;else if(page==='book')content=<BookingFlow service={selectedService} services={services} setPage={setPage} addBooking={addBooking}/>;else if(page==='bookings')content=<Bookings bookings={bookings} setPage={setPage} setSelectedBooking={setSelectedBooking}/>;else if(page==='details')content=<Details booking={selectedBooking} setPage={setPage}/>;else if(page==='track')content=<Track setPage={setPage}/>;else content=<Profile setPage={setPage} onLogout={logout}/>;const showNav=['home','bookings','track','profile'].includes(page);return <div className="app-shell"><div className="phone-app">{content}{showNav&&<BottomNav active={page} setPage={setPage}/>}</div></div>}
